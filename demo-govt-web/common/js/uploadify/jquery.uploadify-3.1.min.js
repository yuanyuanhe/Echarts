...
//    alert(this.settings.upload_url);
...
(function (c) {
    ...
    var a = {
        onDialogOpen: function () {
            var d = this.settings;
            this.queueData.errorMsg = "文件无法上传：";
            ...
            for (var f in this.queueData.files) {
                ...
                if (e.uploaded != true && e.name == g.name) {
                    var d = confirm('文件名"' + g.name + '" 的文件已经在队列中了。\n你想替换队列中已有的文件吗?');
                    if (!d) {
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                switch (g) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件"' + d.name + '"超出了最大限制(' + e.fileSizeLimit + ").";
                        break;
            ...
            if (e.checkExisting) {
                c.ajax({
                    type: "POST", async: false, url: e.checkExisting, data: {filename: d.name}, success: function (h) {
                        if (h == 1) {
                            var g = confirm('文件名"' + d.name + '" 的文件已经在队列中了。\n你想替换队列中已有的文件吗?');
                            if (!g) {
